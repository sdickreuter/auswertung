...
min_y = int(Pabsh.shape[1]/2)-100
max_y = int(Pabsh.shape[1]/2)+100

Pabsh = Pabsh[:,min_y:max_y,2]
Pabsv = Pabsv[:,min_y:max_y,2]
...
grid[0].text(0.08, 0.92, '(a)', horizontalalignment='center',verticalalignment='center', transform=grid[0].transAxes, fontdict=font)
grid[0].set_axis_off()
...
grid[1].text(0.08, 0.92, '(b)', horizontalalignment='center',verticalalignment='center', transform=grid[1].transAxes, fontdict=font)
grid[1].set_axis_off()
...
#plt.show()
plt.savefig(path + "Fig4.png", dpi=1200)
plt.savefig(path + "Fig4.eps", dpi=1200)
plt.close()