...
for array in arrays:
    ...
    savedir = path + sample+'/'+array + '_plots/'
    fname = path + sample + '/' + sample + "_" + array + ".png"
    # fname = path + "test.png"
    print(savedir)
    print(fname)
    ...
    def plot_particles(image1, image2, image3, fname):
        ...
        ax2.imshow(image3, interpolation='nearest')
    ...
    thresh = fdata.max()*0.20#threshold_otsu(fdata)
    bin = fdata > thresh
    ...
    plt.imshow(labeled)
    plt.savefig(savedir+"blobdetect.png",dpi=1200)
    plt.close()
    ...
    for i in range(n):
        ...
        # particles[i] = area[i] /
        plot_particles(sub, distance, labeled, savedir + ids[i] + "_detection.png")
    ...
    f = open(path + sample + "/" + sample + "_" + array + "_particles_SEM.csv", 'w')
