...
for sample in samples:
    ...
    for i in range(len(files)):
        ...
        #    p0 = last_popt
        #else:
        p0 = [-0.004,  0.03,0.005, 0.005, 0.005,  350,510, 575, 620,  150,50, 50,50]

        bounds = [(-np.inf,0,0,0,0,                        300,450,500,580,  10,10,10,10),
                  (np.inf,np.inf,y.max(),y.max(),y.max(),  450,600,750,900,  1000,1000,1000,1000)]
        ...
        try:
            ...
            amp_err =  perr[[2,3,4]]
            sigma = popt[[10,11,12]]
            sigma_err = perr[[10,11,12]]

            sorted_ind = np.argsort(x0)
            x0 = x0[sorted_ind]
            x0_err = x0_err[sorted_ind]
            amp = amp[sorted_ind]
            amp_err = amp_err[sorted_ind]
            sigma = sigma[sorted_ind]
            sigma_err = sigma_err[sorted_ind]

            f = open(savedir + "fitted/" + file[:-4] + ".csv", 'w')
            f.write("x0,x0_err,amp,amp_err,sigma,sigma_err" + "\r\n")
            for i in range(len(x0)):
                f.write(str(x0[i]) + ',' + str(x0_err[i]) + ',' + str(amp[i]) + ',' + str(amp_err[i]) + ',' + str(
                    sigma[i]) + ',' + str(sigma_err[i]) + "\r\n")
