...
def gen_training_data(num, inputs,outputs, filename):
    num_lorentz = 2#int(outputs / 1)
    print(num_lorentz)
    x = np.linspace(0,1,inputs)

    x0s = np.linspace(0, 1, outputs)#100)
    ...
    for i in range(num):
        ...
        for j in range(n):
            ...
            buf_ind = np.random.randint(0,len(x0s)-1,1)
            x0[j] = x0s[buf_ind]
            x0_inds = np.append(x0_inds,buf_ind)
...
nn.create_standard_array((inputs,int(inputs*1.0),outputs))
#nn.create_standard_array((inputs,int(inputs*1.0),outputs))
...
nn.train_on_file(filename,20000,250,0.001)
