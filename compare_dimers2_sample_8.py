...
for a in arrays:
    ...
    d = pd.read_csv(path+a+'/particles_SEM.csv', delimiter=',')
    for i in range(d.shape[0]):
        if d.particles[i] >= 2.0:
            if (d.rdiff[i] < 0.27) & (d.area[i] > 300):
            #if True:
                ...
                sem_ids.append(a+'_'+d.id[i])
                approved.append(a+'_'+d.id[i])
            else:
                rejected.append(a+'_'+d.id[i])
        else:
            rejected.append(a+'_'+d.id[i])
    ...
    spec_path = '/home/sei/Auswertung/'+sample+'/'+a+suffix+'/specs/'
    peak_path = '/home/sei/Auswertung/'+sample+'/'+a+suffix+'/fitted/'
        ...
        for entry in it:
            if not entry.name.startswith('.') and not entry.is_dir():
                if re.fullmatch(r"([a-zA-Z]{1}[0-9]{1}(.csv))", entry.name) is not None:
                    ...
                    spec_ids.append(a+'_'+entry.name[:-4])
...
sem_inds = np.array(sem_inds,dtype=np.int)
spec_inds = np.array(spec_inds,dtype=np.int)
...
for i, spec in zip(range(len(specs)),specs):
    ...
    peaks[i] = peakdata[2,0]
    peaks_err[i] = peakdata[2,1]

    widths[i] = peakdata[2,4]
    widths_err[i] = peakdata[2,5]
    ...
    labels_waterfall.append(labels[i]+' '+str(round(dist[i],1))+'nm')
...
ax1.set_ylabel("Resonanz-Wellenlänge / nm")
ax1.set_xlabel("Abstand / nm")
