...
for i,file in enumerate(files):
    ...
    p1, p2 = np.percentile(fdata, (5, 98))
    ...
    #p1, p2 = np.percentile(fdata, (2, 99))
    #fdata = exposure.rescale_intensity(fdata, in_range=(p1, p2))
    ...
    #threshs = np.linspace(thresh*1.0,thresh*2,200)
    threshs = np.linspace(0.0, 0.8, 200)

    d = np.zeros(len(threshs))
    ...
    for j,thresh in enumerate(threshs):
        ...
        d[j] = find_particles(bin)
        #print(str(d[j])+' ', end='',flush=True)
