...
for n,sim in enumerate(sims):
    ...
    if plot_details:
        ...
        for ind in indexes_charge:
            ...
            # #colormap[:, -1] = 0.499*signal.sawtooth(2 * np.pi * val1/val1.max(),0.5)+0.501
            # colormap[:, -1] = 0.5
            # poly1.set_facecolor(colormap)
            # poly1.set_edgecolor(None)
            #
            # poly2 = plot_trimesh(verts2,faces2,val[len(faces2):])
            # val2 = np.array(val[len(faces2):])  # To be safe - set_array() will expect a numpy array
            # val2_norm = (val2-val2.min()) / (val2.max() - val2.min())
            # colormap = cm.seismic(val2_norm)
            # # val2_norm = val2/np.abs(val2).max()
            # # colormap[:, -1] = 0.5*signal.sawtooth(2 * np.pi * val2_norm+np.pi,0.5)+0.5
            # colormap[:, -1] = 0.5
            # poly2.set_facecolor(colormap)
