...
for n,sim in enumerate(sims):
    ...
    #for m,ind in enumerate(indexes_sca):
    #    plt.text(wl[ind],scat[ind],modelabels[m]+': '+str(int(round(wl[ind]))),zorder=10)

    if len(indexes_sca) == 3:
        modelabels = ['III','II','I']
        # tx = [wl[indexes_sca[0]]-0.03*wl.max(),wl[indexes_sca[1]]-0.03*wl.max(),wl[indexes_sca[2]]-0.03*wl.max()]
        # ty = [scat[indexes_sca[0]]-0.1*scat.max(),scat[indexes_sca[1]]+0.02*scat.max(),scat[indexes_sca[2]]+0.02*scat.max()]
        # texts = []
        # for m, ind in enumerate(indexes_sca):
        #     texts.append(plt.text(tx[m],ty[m], modelabels[m]+' '+str(int(round(wl[ind])))+' nm'))
        tx = [wl[indexes_sca[0]]-0.03*wl.max(),wl[indexes_sca[1]]-0.03*wl.max(),wl[indexes_sca[2]]+0.03*wl.max()]
        ty = [scat[indexes_sca[0]]-0.2*scat.max(),scat[indexes_sca[1]]+0.2*scat.max(),scat[indexes_sca[2]]+0.2*scat.max()]
        for m, ind in enumerate(indexes_sca):
            plt.annotate(modelabels[m]+': '+str(int(round(wl[ind])))+' nm', xy=(wl[ind], scat[ind]), xytext=(tx[m], ty[m]),
                        #arrowprops=dict(facecolor='black', shrink=0.05,width=1.0,headwidth=2.0,frac=0.2),
    ...
    plt.savefig(savedir + sim[:-4] + "_scattering.pgf")
    #plt.show()
    plt.close()
    ...
    if len(indexes_charge) == 3:
        ...
        # for m, ind in enumerate(indexes_sca):
        #     texts.append(plt.text(tx[m],ty[m], modelabels[m]+' '+str(int(round(wl[ind])))+' nm'))
        tx = [wl[indexes_charge[0]]-0.1*wl.max(),wl[indexes_charge[1]]-0.03*wl.max(),wl[indexes_charge[2]]+0.05*wl.max()]
        ty = [charge[indexes_charge[0]]+0.1*charge.max(),charge[indexes_charge[1]]+0.2*charge.max(),charge[indexes_charge[2]]-0.2*charge.max()]
        for m, ind in enumerate(indexes_charge):
            plt.annotate(modelabels[m]+': '+str(int(round(wl[ind])))+' nm', xy=(wl[ind], charge[ind]), xytext=(tx[m], ty[m]),
                        #arrowprops=dict(facecolor='black', shrink=0.05,width=1.0,headwidth=2.0,frac=0.2),
                         arrowprops=dict(arrowstyle="->",
                                         connectionstyle="arc3"),
                        )
    ...
    plt.savefig(savedir + sim[:-4] + "_maxcharge.pgf")
    #plt.show()
    plt.close()
...
if fit_peaks:
    ...
    plt.savefig(savedir + sim[:-4] + "_spectral_shift.pgf")
    #plt.show()
    plt.close()
    ...
    plt.savefig(savedir + sim[:-4] + "_spectral_shift2.pgf", dpi=400)
    #plt.show()
    plt.close()
    ...
    ax1.set_xlabel("Gap / Diameter")
    plt.tight_layout()
    plt.savefig(savedir + sim[:-4] + "_spectral_broadening.png", dpi=400)
    plt.savefig(savedir + sim[:-4] + "_spectral_broadening.pgf")
    #plt.show()
    plt.close()
