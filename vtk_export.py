...
# modified from shocksolutions.com
...
class VTK_XML_Serial_Unstructured:
    ...
    def snapshot(self, fileName, x=None,y=None,z=None,intensity=None,wavelength=None):
        """
        ARGUMENTS:
        ...
        if wavelength is not None:
            wls = doc.createElementNS("VTK", "DataArray")
            wls.setAttribute("Name", "wavelength")
            wls.setAttribute("NumberOfComponents", "1")
            wls.setAttribute("type", "Float32")
            wls.setAttribute("format", "ascii")
            point_data.appendChild(wls)

            string = self.array_to_string(wavelength)
            wlsData = doc.createTextNode(string)
            wls.appendChild(wlsData)

        # Force vectors
        if intensity is not None:
            ints = doc.createElementNS("VTK", "DataArray")
            ints.setAttribute("Name", "intensity")
            ints.setAttribute("NumberOfComponents", "1")
            ints.setAttribute("type", "Float32")
            ints.setAttribute("format", "ascii")
            point_data.appendChild(ints)

            string = self.array_to_string(intensity)
            intsData = doc.createTextNode(string)
            ints.appendChild(intsData)
...
class VTK_XML_Serial_Structured:
    ...
    def snapshot(self, fileName, x=None,y=None,z=None,intensity=None,wavelength=None):
        ...
        doc = xml.dom.minidom.Document()
        root_element = doc.createElementNS("VTK", "VTKFile")
        root_element.setAttribute("type", "StructuredGrid")
        root_element.setAttribute("version", "0.1")
        root_element.setAttribute("byte_order", "LittleEndian")
        doc.appendChild(root_element)

        # Unstructured grid element
        unstructuredGrid = doc.createElementNS("VTK", "StructuredGrid")
        root_element.appendChild(unstructuredGrid)
