...
class GetxIndices:
    ...
    def onClick(self, event):
        if event.button == 1:
            if event.inaxes:
                ind = find_nearest(self.x,event.xdata)
...
for sample in samples:
    ...
    for i in range(len(files)):
        ...
        while not fit_is_good:

            xIndices = GetxIndices(x,y,file)
            plt.show()
            plt.close()
            ...
            p0 = np.hstack((amps/2, x0s, sigmas,0.0))
            n = int(len(amps))

            upper = np.hstack((np.repeat(max(y), n), x0s+20, np.repeat(1000, n),1))#+np.inf))
            lower = np.hstack((np.repeat(0, n), x0s-20, np.repeat(1, n),-1))#-np.inf))
            bounds = [lower,upper]
            ...
            try:
                ...
                goodfit = GoodorBadPlot(x, y,amps, x0s, sigmas, c,file)
                plt.show()
                plt.close()
                fit_is_good = goodfit.good


                f = open(savedir + "fitted/" + file[:-4] + ".csv", 'w')
                f.write("x0,x0_err,amp,amp_err,sigma,sigma_err,c,c_err" + "\r\n")
                for j in range(len(amps)):
                    f.write(str(x0s[j])+','+str(x0s_err[j])+','+str(amps[j])+','+str(amps_err[j])+','+str(sigmas[j])+','+str(sigmas_err[j])+','+str(c)+','+str(c_err)+"\r\n")
